...
public class ItemDetailsActivity extends AppCompatActivity {
    ...
    private void loadItemDetails(int itemId) {
        new Thread(() -> {
            currentItem = db.itemsDao().getItemById(itemId);
            runOnUiThread(() -> {
                if (currentItem != null) {
                    itemHeader.setText("ID: " + currentItem.getId() + ", Name: " + currentItem.getItemName());
    ...
    private void onSaveButtonClick() {
        new Thread(() -> {
            ...
            //should deal with multiple entries at the same time if this was not a single user app
            currentItem.setQuantity(currentQuantity + quantityDifference);

            currentItem.setLocation(itemLocation.getText().toString());

            currentItem.setAlertLevel(alertLevel);
            db.itemsDao().update(currentItem);
    ...
    private void onDeleteButtonClick() {
        new AlertDialog.Builder(this)
                ...
                .setPositiveButton(android.R.string.yes, (dialog, which) -> {
                    new Thread(() -> {
                        db.itemsDao().delete(currentItem);
                        runOnUiThread(() -> {
